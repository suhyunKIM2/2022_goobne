...
<body id="page-top">
            ...
            <div id="content">
	            ...
	            <div class="container-fluid">
                    <div class="d-sm-flex align-items-center justify-content-between mb-4 container_auto">
                        <h1 class="h3 mb-0 text-gray-900">이벤트 게시판</h1>
                    </div>
                    <div class="col-xl-4 col-lg-5">
                    <ul class="nav nav-tabs">
                      <li class="nav-item">
                        <a href="javascript:toggleClass('now');" id="nowEvent" class="nav-link active">진행중인 이벤트</a>
                      </li>
                      <li class="nav-item">
                        <a href="javascript:toggleClass('end');" id="endEvent" class="nav-link">종료된 이벤트</a>
                      </li>
                    </ul>
                    <div class="tab-content">
                      ...
                      <ul class="list_inline" style="text-align: right;    margin-top: 2%;">
                            <li style="float:left;">
                          ...
                          <table class="table table-bordered table_main board_table_list" id="dataTable" width="100%" cellspacing="0">
                            <colgroup>
                                ...
                                <col width="10%">
                                <col width="*">
                                <col width="10%" style="min-width:147px;">
                                <col width="15%">
                                <col width="10%">
                                <col width="10%">
                                <col width="10%">
                            ...
                            <thead>
                                <tr>
                                	<th class="bg-gray-200"><input type="checkbox" name="chk" id="All"><label for="All" ></label></th>
  ...
  <!-- Start bootstrap footer script  -->
	...
	function toggleClass(date){
		...
		if(date == 'end'){
			$("#endEvent").addClass("active");
			$("#nowEvent").removeClass("active");
		} else if(date == 'now'){
			$("#nowEvent").addClass("active");
			$("#endEvent").removeClass("active");
		}
		srchBrdList(1);
	...
	function srchBrdList(pageNum) {
		...
		if($("#nowEvent").hasClass("active")){
            obj.eventGb = 'now_date';
        }
		if($("#endEvent").hasClass("active")){
		...
		var url = '<%=SystemConstant.SITE_PATH%>/brd/event/srch_list'
				...
				success : function(obj){
					...
					if(brdList.length>0){
						...
						for (var i in brdList) {
							...
							if(brdList[i].total_cmt_cnt != null){
								console.log(brdList[i].total_cmt_cnt);
								brdHtml += '<span class="btn btn-danger btn-circle btn-sm" style="border-radius: 5px;width:auto;">';
								brdHtml += '댓글 ' + brdList[i].total_cmt_cnt; //댓글겟수
								brdHtml += '<i class="fa fa-comment fa" style="margin-left:3px;"></i></span>';
							}
							...
							brdHtml += "</td>";
							brdHtml += '<td class="small_font">';
							brdHtml += brdList[i].s_time + " - " + brdList[i].e_time; //이벤트기간
							brdHtml += "</td>";
							brdHtml += '<td class="small_font">';
						...
						brdHtml += '<tr><td colspan="9">조회된 이벤트가 존재하지 않습니다.</td></tr>'
					}
	...
	function brdView(seq){
    	$('#seq').val(seq);
    	document.brdForm.action='<%=SystemConstant.SITE_PATH%>/brd/event/view'; 
	    document.brdForm.submit();
    }
	
	function brdWrite(){
		$('#seq').val('');
    	document.brdForm.action='<%=SystemConstant.SITE_PATH%>/brd/event/write'; 
	    document.brdForm.submit();
	}
